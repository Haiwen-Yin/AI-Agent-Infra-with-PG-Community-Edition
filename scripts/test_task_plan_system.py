...
Usage:
    chmod +x test_task_plan_system.py
    ./test_task_plan_system.py [--host HOST] [--port PORT] [--db DB] [--user USER] [--pass PASSWORD]

Default: host=10.10.10.131, port=5432, db=memory_graph, user=postgres
"""
...
class TaskPlanTestSuite:
    def __init__(self, host="10.10.10.131", port=5432, dbname="memory_graph", 
        ...
        self.sql_file = os.path.join(self.skill_dir, "scripts", "init_task_plan_system.sql")
    ...
    def deploy_schema(self):
        ...
        result = subprocess.run(
            f"psql -h {self.host} -p {self.port} -U {self.user} -d postgres -c \"SELECT 1 FROM pg_database WHERE datname='{self.dbname}'\" > /dev/null 2>&1 || psql -h {self.host} -p {self.port} -U {self.user} -c 'CREATE DATABASE {self.dbname}'",
    ...
    def test_crud_operations(self):
        ...
        try:
            ...
            """, ("Test Deployment", "deployment", "Deploy test database", 
    ...
    def run_all_tests(self):
        ...
        self.print_header("🧪 TASK PLAN SYSTEM TEST SUITE")
