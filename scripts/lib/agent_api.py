"""AI Agent Infra v4.0.0 - PG Community Edition - Agent API
...
def register_agent_via_admin(
    ...
    # PostgreSQL uses a shared DB role + RLS driven by app.current_agent_id,
    # so there is no per-agent End User password. We still emit a credential
    # blob (using the shared role) so agent_bootstrap.py works uniformly
    ...
    credential_data = {
        ...
        "dsn": "{}:{}:{}".format(db_cfg.host, db_cfg.port, db_cfg.dbname),
        "agent_id": agent_id,
        "db_type": "pg",
    }
...
def recover_agent_via_admin(
    ...
    # Re-emit shared DB credentials so the recovered agent can reconnect.
    from .config import get_config
    db_cfg = get_config().database
    credential_data = {
        "username": db_cfg.user,
