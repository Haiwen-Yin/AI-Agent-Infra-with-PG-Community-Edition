"""AI Agent Infra v3.10.2 - PG Community Edition - Property Graph API
...
def get_neighbors(
    ...
    if direction in ("outgoing", "both"):
        ...
        sql = f"""
            SELECT ent.entity_id AS neighbor_id,
                   ent.entity_type AS neighbor_type,
                   ent.title AS neighbor_title,
...
def get_reachable(
    ...
    sql = f"""
        SELECT ent.entity_id, ent.title, ent.entity_type
        FROM entity_edges e
        JOIN entities ent ON ent.entity_id = e.target_id
        WHERE {full_where}
        LIMIT :lim
...
def get_shortest_path(
    ...
    sql = """
        WITH RECURSIVE path_cte AS (
            ...
            SELECT e2.target_id AS entity_id,
                   e2.edge_type,
                   e2.strength,
                   ent2.title,
                   ent2.entity_type,
                   p.hop + 1 AS hop,
                   p.visited || e2.target_id
            FROM path_cte p
            JOIN entity_edges e2 ON e2.source_id = p.entity_id
            JOIN entities ent2 ON ent2.entity_id = e2.target_id
            WHERE p.hop < :max_hops
              AND NOT (e2.target_id = ANY(p.visited))
        )
        SELECT entity_id, title, entity_type, edge_type, strength, hop
        FROM path_cte
        WHERE entity_id = :tgt
        ORDER BY hop
        LIMIT 1
    """
    params = {"src": source_id, "tgt": target_id, "max_hops": max_hops}
...
def find_similar_entities(
    ...
    sql = f"""
        SELECT ent.entity_id, ent.title, ent.entity_type,
               ent.category, ent.importance
        FROM entity_edges e
        JOIN entities ent ON ent.entity_id = e.target_id
        WHERE e.source_id = :eid AND ent.entity_id <> :eid
        ORDER BY ent.importance DESC
        LIMIT :lim
    """
...
def find_communities(
    ...
    type_condition = f"AND v.entity_type = :etype" if entity_type else ""
...
def find_causes(entity_id, depth=3):
    rows = execute_query(
        "SELECT e.target_id, e.edge_type, e.strength, e.metadata, e.created_at, ent.title, ent.entity_type FROM entity_edges e JOIN entities ent ON ent.entity_id = e.source_id WHERE e.target_id = :eid AND e.edge_type = 'CAUSES' LIMIT :limit",
...
def trace_provenance(entity_id, depth=5):
    ...
    for _ in range(depth):
        rows = execute_query(
            "SELECT e.source_id, e.edge_type, ent.title, ent.entity_type, ent.category FROM entity_edges e JOIN entities ent ON ent.entity_id = e.source_id WHERE e.target_id = :eid AND e.edge_type IN ('DERIVED_FROM', 'DERIVED_FROM_DATA', 'SUPERSEDES', 'SUPERSEDED_BY') LIMIT 1",
            {"eid": current_id},
        )
        if not rows:
            break
...
def update_trust(agent_id, target_id, group_id, success):
    ...
    if not row:
        return False
    new_strength = max(0.0, min(max_val, (float(row.get('strength') or 0.5)) + delta))
    from .connection import execute
...
def get_task_lineage(entity_id):
    ...
    for _ in range(10):
        rows = execute_query(
            "SELECT e.source_id, e.edge_type, ent.title, ent.entity_type FROM entity_edges e JOIN entities ent ON ent.entity_id = e.source_id WHERE e.target_id = :eid AND e.edge_type IN ('PRODUCED_ARTIFACT', 'CONSUMED_ARTIFACT', 'FEEDS_INTO') LIMIT 1",
            {"eid": current_id},
        )
        if not rows: break
        r = _row_to_dict(rows[0])
...
def trace_memory_origin(entity_id):
    ...
    for _ in range(10):
        rows = execute_query(
            "SELECT e.source_id, e.edge_type, ent.title, ent.entity_type FROM entity_edges e JOIN entities ent ON ent.entity_id = e.source_id WHERE e.target_id = :eid AND e.edge_type IN ('PROMOTED_TO', 'MERGED_INTO', 'SUPERSEDED_BY') LIMIT 1",
            {"eid": current_id},
        )
        if not rows: break
        r = _row_to_dict(rows[0])
