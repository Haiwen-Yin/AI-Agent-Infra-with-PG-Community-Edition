-- v4.4.9 Security Domain member enforcement for governed Channel resources.
-- Identity resolves first from a dedicated database role. The custom setting
-- is only a verification value inside a bounded transaction and cannot
-- establish identity for an unmapped role.
