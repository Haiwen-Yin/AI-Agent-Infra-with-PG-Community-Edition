"""AI Agent Infra v3.10.1 - PG Community Edition - Admin/Agent Separation Tests
...
def test_credential_distribution_encrypt_decrypt():
    ...
    assert decrypted["host"] == "10.10.10.130"
...
def test_save_load_agent_config():
    ...
    try:
        ...
        assert loaded["host"] == "10.10.10.130"
...
def test_full_registration_and_local_save():
    ...
    cred_data = {
        "username": "FULL_FLOW_EU",
        "password": "flow_test_pwd",
        "host": "10.10.10.130",
        "port": "5432",
        "dbname": "ai_agent",
    }
    encrypted = encrypt_credential_for_distribution(cred_data, ADMIN_TOKEN)
    ...
    try:
        ...
        assert loaded["host"] == decrypted["host"]
...
def test_recover_agent_via_admin():
    ...
    execute(
        "UPDATE agent_registry SET last_seen_at = CURRENT_TIMESTAMP - INTERVAL '1 hour' WHERE agent_id = %s",
        [RECOVERY_AGENT],
    )
    ...
    assert result.get("agent_id") == RECOVERY_AGENT
