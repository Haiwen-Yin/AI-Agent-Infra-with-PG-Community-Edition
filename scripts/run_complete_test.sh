#!/bin/bash
# Complete Test Script for memory-pg18-by-yhw v0.3.2 Task Plan System
# Runs on remote PG server via SSH
...
# Step 1: Copy SQL file to remote server
echo ""
...
# Step 2: Run all tests via SSH in one session
echo ""
echo "📋 STEP 2-6: Running complete test suite on PG server..."
...
echo "================================================================================"
pg_ctl status 2>&1 | head -5 || /usr/local/pgsql/bin/pg_ctl start -l /var/lib/pgsql/data/log/postgresql.log -w && echo "PG started"

# Verify PG version
echo ""
echo "📊 PostgreSQL Version:"
psql -d postgres --version 2>/dev/null || psql --version
...
if [ "$DB_EXISTS" = "0" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        psql -d postgres -c "CREATE DATABASE memory_graph;" 2>&1
...
echo "================================================================================"
psql -d memory_graph -f /tmp/init_task_plan_system.sql 2>&1 | grep -E "(CREATE|ALTER|INSERT)" || psql -d memory_graph -f /tmp/init_task_plan_system.sql
...
WHERE table_name IN ('task_plans', 'task_steps', 'task_context_snapshots', 
                      'task_tool_calls', 'task_dependencies') AND schemaname = 'public';

SELECT tablename, pg_size_pretty(pg_relation_size(tablename)) as size
FROM pg_tables 
