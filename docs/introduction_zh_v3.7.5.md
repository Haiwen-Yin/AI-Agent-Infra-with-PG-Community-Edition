# AI Agent Infra with PostgreSQL — 社区版 v3.7.5

**版本**: v3.7.5 | **日期**: 2026-06-28 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.7.5 修复v3.7.4代码审查中发现的问题：connection.py从oracledb重写为psycopg2、10个模块Oracle SQL适配PostgreSQL、monitor_api列名修正、orchestrator空壳执行修复、event_bus安全加固、message_api软删除状态修正(FAILED→DELETED)。基于v3.7.4引入的6大扩展方向，构建完整的 AI Agent 基础设施。适配 PostgreSQL 18.3 生态：psycopg2 驱动、pgvector 向量搜索、Apache AGE 属性图、pg_cron 定时任务、PL/Python3u 存储过程、pgcrypto 数据库加密、Row Security Policies 行级安全。
