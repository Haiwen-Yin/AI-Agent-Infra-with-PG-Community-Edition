# AI Agent Infra with PostgreSQL — 社区版 v3.8.0

**版本**: v3.8.0 | **日期**: 2026-07-02 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.8.0 完成完整多Agent集成测试：修复connection.py _convert_params参数顺序错误（重复:param未正确替换）、execute_query_one缺失commit导致PL/pgSQL函数DML丢失、1_schema.sql CREATE POLICY双重ON子句语法错误、loop_audit外键引用分区表复合主键不兼容、user_manager.authenticate缺少v_salt变量声明、event_bus/monitor_api列名错误、PROMPT命令转注释。在全新PG 18.3数据库上完成5阶段部署验证：69张表、176个函数、4个Business Agent注册、15/15功能测试全部通过。
