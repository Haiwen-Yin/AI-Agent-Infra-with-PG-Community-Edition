# AI Agent Infra with PostgreSQL — 社区版 v3.7.3

**版本**: v3.7.3 | **日期**: 2026-06-19 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.7.0 引入循环工程（Loop Engineering）——第4代 AI 工程方法论（继提示词工程、上下文工程、Harness 工程之后），由 Peter Steinberger 于2026年6月提出。新增4张循环表、loop_manager PL/pgSQL schema、loop_api.py Python模块、6种评估类型、生命周期钩子和3个 pg_cron 作业。v3.7.1 新增循环工程协同集成：规格驱动循环、Task-Loop 绑定、协作循环、SPEC_VALIDATION 与 AGGREGATE 评估类型、Skill-Triggered Loop。同时修复会话持久化、PG 循环 API 兼容性等问题。v3.7.2 文档一致性修正：loop_manager 函数数、loop_api 公共函数数、LOOP_CLEANUP 调度、PL/SQL→PL/pgSQL 术语、ENTITIES 分区数、评估类型数、架构图对齐等。v3.7.3 部署修复：RLS 策略硬编码、配置优先级、Embedding 模型提示。适配 PostgreSQL 18.3 生态：psycopg2 驱动、pgvector 向量搜索、Apache AGE 属性图、pg_cron 定时任务、PL/Python3u 存储过程、pgcrypto 数据库加密、Row Security Policies 行级安全。
...
| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.7.3** | 2026-06-23 | 部署修复：RLS策略硬编码schema_owner、配置优先级、Embedding模型提示 |
| **v3.7.2** | 2026-06-19 | 文档一致性修正：loop_manager ~33→~22、loop_api 32公共函数、LOOP_CLEANUP Weekly Sunday 06:00、PL/SQL→PL/pgSQL、ENTITIES 分区7→8、评估类型4→6、架构图对齐 |
...
## 七、Bug 修复（v3.6.1 → v3.7.0）及 v3.7.3 新功能
...
## 循环工程（Loop Engineering）[NEW v3.7.3]
