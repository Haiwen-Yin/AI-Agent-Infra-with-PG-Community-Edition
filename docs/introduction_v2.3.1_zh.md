...
> **统一 AI 智能体记忆系统** — 基于知识图谱、多智能体协作、任务规划、Harness 模板引擎、规格驱动开发、智能体弹性管理、协作组、嵌入 Python API、属性图 API、工作空间与上下文恢复，及 Web 可视化,构建于 PostgreSQL 18 之上。
...
**v2.3.1 是 v2.3.1 的功能增强升级**，新增嵌入 Python API（embedding_api.py，12 个函数）和 EMBEDDING_GENERATION_JOB 调度作业，与 v2.3.1 向后兼容，为增量式扩展。

---

## 二、v2.3.1 核心变更 — 规格驱动开发、智能体弹性管理、协作组

| v2.2.1 痛点 | v2.3.1 方案 | 效果 |
|-------------|------------|------|
...
- 所有枚举列加 CHECK 约束
- 种子数据:SYSTEM_CONFIG（版本 2.3.1）、SYSTEM_USERS（admin 账户）
...
13 个自动化调度作业（pg_cron，v2.3.1 新增 EMBEDDING_GENERATION_JOB）:
...
| EMBEDDING_GENERATION_JOB | 每 2 小时 | 为 MEMORY/KNOWLEDGE 实体生成嵌入向量 |
...
| embedding_api.py | **v2.3.1 新增** 嵌入生成、存储、搜索、批量、统计（12 个函数，pgvector + pg-embedding-gen-by-yhw） |
...
```
PostgreSQL Memory System v2.3.1 - 全量测试套件
============================================================
  Connection:  6/6 PASS
  Memory:     16/16 PASS
...
| test_embedding.py | 19 | **v2.3.1 新增** 嵌入生成/存储/搜索/批量/统计/混合/多类型 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v2.3.1** | 2026-05-26 | 嵌入 Python API、EMBEDDING_GENERATION_JOB、19 个嵌入测试 |
| **v2.3.1** | 2026-05-24 | 规格驱动开发、智能体弹性管理、协作组 |
