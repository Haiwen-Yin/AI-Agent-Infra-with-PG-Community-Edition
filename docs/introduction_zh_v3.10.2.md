# AI Agent Infra with PostgreSQL — 社区版 v3.10.2

**版本**: v3.10.2 | **日期**: 2026-07-16 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.10.2 加密增强：每Agent独立加密密钥（system_config存储+admin_token分发）、config.json启动时自动加密（database+llm+model_routing）、密钥轮换API（全局+单Agent）。PG专属修复：凭证加密Bug（_get_crypto_key随机密钥）、Business Agent模式、_convert_params空元组修复、Oracle SQL语法替换。544/544测试通过。

> v3.10.0 通用属性图扩展：8个领域30+新图函数（知识因果、Agent协作组内动态信任、任务编排、Skill依赖、审批传播、数据流、记忆演化、Loop迭代），23个新边类型，信任配置通过SYSTEM_CONFIG动态调整。修复：PG purge_audit_logs列名、AGE create_graph缺失。
...
| **v3.8.0** | 2026-07-02 | 多Agent集成测试：_convert_params参数顺序修复、execute_query_one commit修复、CREATE POLICY双重ON修复、loop_audit FK修复、authenticate v_salt修复、event_bus/monitor_api列名修复、PROMPT转注释；5阶段部署验证全通过(69表/176函数/15功能测试) |
| **v3.7.5** | 2026-06-28 | Bug修复：connection.py重写(oracledb转psycopg2)、10模块Oracle SQL转PostgreSQL、monitor_api列名修正、orchestrator空壳修复、event_bus安全、message_api DELETED状态 |
...
| **v3.7.0** | 2026-06-18 | 循环工程（第4代AI方法论）：4张循环表、loop_manager schema、loop_api.py、6种评估类型、生命周期钩子、3个 pg_cron 作业 |
...
## 七、Bug 修复（v3.6.1 → v3.7.0）及 v3.7.5 新功能
...
## 循环工程（Loop Engineering）[NEW v3.7.5]
