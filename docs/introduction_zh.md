# AI Agent Infra with PostgreSQL — 社区版 v4.2.0
...
- **Python 3.8+，需安装 psycopg2 2.9+**：`pip install psycopg2-binary>=2.9`（发布包 `vendor/` 已预装）。
...
Lease Token，不会获得 Schema Owner 凭证；过期 fencing token 不能覆盖更新
后的执行结果。
