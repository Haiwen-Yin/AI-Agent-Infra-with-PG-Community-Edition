# AI Agent Infra with PostgreSQL — 社区版 v4.4.11
...
> v4.3.7 确定性初始化与嵌入契约：针对已准备好的目标库，包内 Bootstrap Deployment Agent 校验清单后通过 Python 驱动执行受约束 SQL，不依赖外部 Agent、`psql` 或 LLM 输出作为部署权限。Embedding Profile、不可变 Contract、Space 和 Binding 统一约束向量维度、模型与预处理；支持平台托管、企业直连、企业代理、预计算导入和禁用五种模式。
...
> v4.4.2 Embedding 与图工程运行：管理员先测试模型服务，平台自动识别向量维度并原子化维护 Profile、Contract、默认 Space 与 Binding；统一归一化规则由平台契约强制执行，后续结果为只读状态，不再要求手工创建契约、空间或迁移任务。知识清单与图投影按已认证主体及其有效可见范围查询。Graph Production Profile 按能力控制 Graph Core、检查、A2A、OTLP、回放和动态迁移；稳定能力可生产使用，其余默认关闭或受控启用。
...
- **本技术发行包**：AI Agent Infra with PostgreSQL（PostgreSQL 数据库适配器）

本文档描述川序产品在 PostgreSQL 18.3+ 上的技术发行包。`AI Agent Infra with DB`
...
本项目的核心设计理念是：**将 AI Agent 运行所需的一切基础设施——记忆、知识、身份、技能、安全、分支——统一收敛于一个数据库内核之中**，利用 PostgreSQL 的 `pgvector` 向量搜索、`pg_trgm` 模糊匹配、Row-Level Security（RLS）行级安全、`pgcrypto` 列级加密、`pg_cron` 定时任务等原生扩展，在数据库层实现基础设施的完整闭环，而非依赖外部微服务拼装。
...
| 加密存储 | PBKDF2+AES-256-GCM、pgcrypto 数据库内加密、主密钥管理、自动加密 |
| 数据库访问安全 | 规范约束 + 最小权限用户 + Row-Level Security (RLS) + 审计 + 脱敏 |
...
PostgreSQL 版本自 v3.6.1 起独立发布，并持续扩展 Agent 管理、工作空间、规格驱动开发、Skill 分发、身份认证、上下文分支等能力。当前版本以独立 LOGIN 角色、RLS、Admin Token、加密凭证分发和 Recovery Code 构成 Admin/Business Agent 隔离体系。
...
| **v4.3.7** | 2026-08-10 | 新增 Bootstrap Deployment Agent、加密部署证据、无外部 Agent/`psql` 的受校验初始化，以及 Embedding Profile/不可变 Contract/Space/Binding 治理；向量按空间与契约隔离，托管批量任务使用租约 Worker |
...
> 以下列出 PostgreSQL 版本自 v3.6.1 起的独立发布历史。
...
| **v3.10.2** | 2026-07-17 | 企业加密增强：每 Agent 独立密钥、config.json 自动加密、密钥轮换 API、Business Agent 身份隔离、统一 encrypt_config.py CLI 与 Portal Markdown 渲染 |
...
| **v3.8.0** | 2026-07-02 | LOOP_MANAGER、凭证加密、schema 前缀与授权脚本稳定性修复 |
| **v3.7.5** | 2026-06-28 | orchestrator、event_bus、message_api 与企业版授权稳定性修复 |
...
| 加密凭证存储 | LDAP BIND_CREDENTIAL 加密、AGENT_CREDENTIALS 加密、config.json 自动加密、pgcrypto 数据库内加密 |
...
| **数据库对象** | | |
| 基础 Schema 表 | 71 | 71 |
| PL/pgSQL API 函数 | 118 | 118 |
| 调度定义 | 以 `3_jobs.sql` 与版本迁移为准 | 以 `3_jobs.sql` 与版本迁移为准 |
...
### 4.1 PostgreSQL 数据库基础

本项目深度利用 PostgreSQL 18.3+ 的多项原生扩展能力，将基础设施逻辑下沉到数据库内核：

| PostgreSQL 能力 | 应用场景 |
|-----------------|---------|
| **pgvector 向量搜索** | ENTITY_EMBEDDINGS 表存储 VECTOR(1024) 类型嵌入，支持余弦相似度检索 + HNSW/IVFFlat 索引加速 |
| **pg_trgm 模糊匹配** | 跨列三元组检索，实现模糊搜索 + 相关性评分 |
| **全文搜索** | ENTITIES_SEARCH_CTX 全文索引，tsvector + tsquery 跨列检索，ts_rank 相关性评分 |
| **Row-Level Security (RLS)** | 每张表创建 RLS 策略，通过 `app.current_agent_id` 会话变量实现行级隔离 |
| **pgcrypto 列级加密** | LDAP BIND_CREDENTIAL、AGENT_CREDENTIALS 等敏感字段使用 pgcrypto AES 加密 |
| **PL/pgSQL 函数** | 以 SECURITY DEFINER 函数实现 API 层，强制走 PL/pgSQL 接口并执行业务逻辑 |
| **pg_cron 定时任务** | 记忆融合、知识提取、会话清理、审计日志清理等调度作业 |
| **JSONB 类型** | METADATA、CONFIG 等字段使用 JSONB 存储，支持 JSONB 操作符查询和 GIN 索引 |
...
| **数据库层** (Database Layer) | 71 张基础 Schema 表 · 118 个 PL/pgSQL API 函数 · RLS · JSONB · 向量索引 · 全文索引 |

**设计原则**：

- 数据库层（PL/pgSQL）处理事务与数据密集逻辑：记忆融合、知识提取、审计检测、分支合并
...
| 列名 | 类型 | 说明 |
|------|------|------|
| ENTITY_ID | VARCHAR(64) | 实体唯一标识 |
| ENTITY_TYPE | VARCHAR(32) | 类型鉴别器，复合主键组成部分 |
| TITLE | VARCHAR(512) | 实体标题 |
| CONTENT | TEXT | 实体内容（大文本） |
| SUMMARY | VARCHAR(2000) | 摘要 |
| CATEGORY | VARCHAR(64) | 分类 |
| IMPORTANCE | INTEGER | 重要性评分（1-10） |
| VISIBILITY | VARCHAR(16) | 可见性（PRIVATE/SHARED/PUBLIC） |
| WORKSPACE_ID | VARCHAR(64) | 所属工作区 |
| CREATED_AT | TIMESTAMP | 创建时间 |

**主键与关联设计**：`ENTITIES` 以 `ENTITY_ID` 标识实体，各元数据表通过外键关联实体。
...
| **fulltext** | 0.25 | ENTITIES_SEARCH_CTX | tsvector + tsquery + ts_rank 全文相关性 |
...
| 列名 | 类型 | 说明 |
|------|------|------|
| ENTITY_ID | VARCHAR(64) | 实体标识，FK 到 ENTITIES |
| SKILL_NAME | VARCHAR(256) | 技能名称 |
| SKILL_VERSION | VARCHAR(32) | 版本号，默认 1.0.0 |
| SKILL_TYPE | VARCHAR(32) | BUILTIN / CUSTOM |
| SKILL_FORMAT | VARCHAR(32) | TEXT / SCRIPT / HYBRID |
| TEXT_CONTENT | CLOB | SKILL.md 文本内容 |
| RESOURCE_URI | VARCHAR(2048) | 资源文件相对路径 |
| RESOURCE_SERVER_HOST | VARCHAR(512) | 服务器主机名 + IP |
| SKILL_DESCRIPTION | CLOB | 技能描述 |
| RUNTIME | VARCHAR(32) | PYTHON / BASH / NODE / OTHER |
...
Skill 数据保存在关系表与 JSONB 字段中，由 Python API 组合为结构化文档；更新操作通过受控 API 完成。
...
加密凭证系统采用本地文件加密（connection_crypto）和 PostgreSQL 数据库内加密函数双轨方案，覆盖数据库连接、API Key、会话签名密钥、LDAP 凭证和 Agent 凭证。
...
| **数据库内加密** | pgcrypto + `db_crypto_*` PL/pgSQL 函数 | LDAP BIND_CREDENTIAL、AGENT CREDENTIALS | SYSTEM_CONFIG 表 | 不依赖本地文件 |
...
| LDAP_CONFIG.BIND_CREDENTIAL | pgcrypto（数据库内加密） | `configure_ldap()` 写入时加密，读取时按权限解密 |
| AGENT_CREDENTIALS.CREDENTIAL_VALUE | pgcrypto（数据库内加密） | 凭证签发与验证使用数据库密钥加密 |
...
### 5.10b PostgreSQL 数据库内加密

PostgreSQL 通过 pgcrypto 扩展和 `db_crypto_encrypt()`、`db_crypto_decrypt()`、`db_crypto_rotate_key()` PL/pgSQL 函数实现 AES 数据加解密与密钥轮换，密钥配置存储在数据库中。

#### 核心特性

- **数据库函数**：使用 pgcrypto `encrypt_iv` / `decrypt_iv` 执行 AES 加解密
- **密钥存储**：密钥配置存储在 SYSTEM_CONFIG 表中
- **访问边界**：仅授权的 SECURITY DEFINER/API 路径可以处理敏感字段
- **密钥轮换**：`db_crypto_rotate_key()` 在事务中重加密活动凭证

#### 与 connection_crypto 的分工

| 维度 | connection_crypto | PostgreSQL 数据库内加密 |
|------|-------------------|-----------|
| 加密对象 | 本地文件（config.json 数据库凭证、API Key、会话签名密钥） | 数据库内数据（LDAP BIND_CREDENTIAL、AGENT CREDENTIALS） |
| 密钥存储 | 本地 master.key 文件 / 环境变量 | SYSTEM_CONFIG 表 |
...
```sql
-- 在受控管理事务中轮换数据库凭证密钥
SELECT db_crypto_rotate_key(:old_key, :new_key);
```
...
| **L2 独立数据库身份** | 每个 Business Agent 使用专属 LOGIN 角色 | 技术层面隔离身份并禁止 Schema Owner 回退 |
| **L3 SECURITY DEFINER** | PL/pgSQL API 函数以受控属主权限执行 | 强制走授权 API 并执行业务逻辑 |
| **L4 Row-Level Security** | RLS 策略 + `app.current_agent_id` 会话变量 + 共享角色 | 声明式行级访问控制 + 零信任 - ✅ **策略已部署并生效** |
| **L5 统一审计** | entity_access_log + 审计触发器 | 审计所有绕过 API 的直接操作 |
| **L6 凭证过滤** | save_context() 自动脱敏 | 防止凭证泄露到上下文存储 |

>
> PostgreSQL Business Agent 使用独立的 NOLOGIN 运行角色和专属 LOGIN 角色：
> - 每个 Business Agent 获得独立数据库登录凭证，`agent_config.json` 只保存该身份。
> - 连接建立后设置由认证上下文派生的 `app.current_agent_id`，RLS 据此过滤数据。
> - Business Agent 连接失败时禁止回退到 Schema Owner。
> - Admin Agent 才能持有 Schema Owner 凭证；Portal 退出和本节点服务重启只回收本节点 Agent。
> - 运行角色为 `NOBYPASSRLS`，专属 LOGIN 角色为 `NOSUPERUSER/NOCREATEDB/NOCREATEROLE/NOBYPASSRLS`。
>

#### PostgreSQL Business Agent 角色创建前提

Admin Agent 使用 Schema Owner 连接为每个 Business Agent 创建独立 LOGIN
角色并授予共享的 `ai_agent_runtime` 运行角色。首次注册前，需要由 DBA 为
Schema Owner 配置以下两项最小角色管理前提：

1. `CREATEROLE`：允许创建每个 Agent 的专属 LOGIN 角色；
2. 已由 DBA 预创建 `ai_agent_runtime` 时，Schema Owner 需要该角色的
   `ADMIN OPTION`，才能将运行角色授予新 LOGIN。
...
不得向 Business Agent 或 `ai_agent_runtime` 授予上述管理权限。生成的专属
...
| 权限 | Schema Owner（部署/管理） | Business Agent（运行时） |
|------|-------------------|---------------------|
| CREATE SESSION | ✓ | ✓ |
| 执行授权 API | ✓ | ✓（SECURITY DEFINER） |
| 表访问 | ✓ | 受 RLS 与对象授权约束 |
| INSERT/UPDATE/DELETE | ✓ | 仅授权表且受 RLS 约束 |
| CREATE/ALTER/DROP | ✓ | ✗ |
...
PG 版本通过 `entity_access_log` 表和审计触发器记录所有绕过 API 的直接操作。当非 AIADMIN 用户直接对关键表（WORKSPACE_CONTEXT、AGENT_REGISTRY、CONTEXT_BRANCHES、SYSTEM_CONFIG）执行 DML 时自动记录审计日志。

查询绕过检测：
```sql
SELECT * FROM UNIFIED_AUDIT_TRAIL 
WHERE entity_type IN ('WORKSPACE_CONTEXT', 'AGENT_REGISTRY', 'CONTEXT_BRANCHES', 'SYSTEM_CONFIG')
...
| 需要加密 | LDAP BIND_CREDENTIAL | pgcrypto 数据库内加密 |
| 需要加密 | AGENT CREDENTIALS | pgcrypto 数据库内加密 |
| 更底层保护 | PRIVATE 记忆明文内容 | PostgreSQL 主机/磁盘/卷加密（部署层） |

#### 存储介质保护

项目负责字段加密、最小权限和 RLS；PostgreSQL 数据目录、备份和卷的静态加密由部署环境提供。生产部署应启用主机或存储层加密，并将其与 pgcrypto 字段加密组合为纵深防御。
...
| DB_CRYPTO | 3 | pgcrypto AES 加解密与密钥轮换 |
...
PostgreSQL 版本通过 JSONB 字段、关系表外键和 Python API 组合 Memory、Knowledge、Workspace、Context、Spec、Collaboration 与 Skill 文档，不依赖数据库文档视图。所有写操作经过受控 API、事务和 RLS 校验。
...
- **PostgreSQL 18.3 或更高** — 验证：`SELECT version();`。需要 `pgvector`、`pg_trgm`、`pgcrypto`、`pg_cron` 扩展。
- **Python 3.14+，需安装 psycopg2 2.9+**：发布包 `vendor/` 已预装，Linuxbrew 不是必需项。
- 部署脚本：`psql -h <host> -p <port> -U <user> -d <dbname> -f scripts/deploy/1_schema.sql`（依次执行 1_schema → 2_api → 3_jobs → 4_harness_templates）
- **RLS 策略授权**：部署前需由 DBA 授予 schema_user 创建 RLS 策略的权限

> ⚠️ **关键**：PostgreSQL 必须为 **18.3** 或更高版本，且必须启用 `pgvector` 与 `pgcrypto` 扩展。

> ⚠️ **关键**：行级安全（RLS）通过 `app.current_agent_id` 会话变量驱动，Business Agent 连接后必须立即执行 `SET app.current_agent_id = '<agent_id>'`。
...
在 `config.json` 的 `database` 段使用 `pool_min` 与 `pool_max` 配置连接池，
默认值分别为 `2` 与 `5`。PostgreSQL 适配器兼容读取这两个统一版本配置名，
因此已有的加密配置在升级后不会悄然回退到其他默认值。Dashboard 的短时并发读取
会在 `pool_max` 以内进行有限等待；持续并发超过上限仍应由运维人员结合 PostgreSQL
连接预算提高 `pool_max`，而不是无限制创建连接。
...
PostgreSQL 18 使用 Apache AGE 实现 Property Graph 投影，`GRAPH_*` 关系表
仍是共享执行事务和恢复的权威来源。PostgreSQL 19 原生 Property Graph 属于
 后续适配方向，不是本版本的前置条件。Worker 只获得受限输入和短期
Lease Token，不会获得 Schema Owner 凭证；过期或撤销的 fencing token 不能
覆盖更新后的执行结果。成功完成会保存请求摘要，同一 token 对同一输出和
证据的重试只返回原有 Checkpoint，不会重复写入状态或下游 Transition；冲突
输出仍会被拒绝。
