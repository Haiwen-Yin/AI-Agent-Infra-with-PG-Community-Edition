# AI Agent Infra with PostgreSQL — 企业版 v4.0.0
...
在持续演进过程中，项目逐步扩展了 Agent 管理、工作空间、规格驱动开发、Skill 分发、身份认证、上下文分支等能力，从单一的记忆系统成长为覆盖 AI Agent 全生命周期的基础设施架构。v3.6.0 引入 Admin/Agent 分离架构：模式系统（standalone/admin/agent）、Admin Token 认证、加密凭证分发、Recovery Code 恢复码、Agent Bootstrap CLI、Skill Token API。v3.6.1 起 PostgreSQL 版本发布，采用 RLS 行级安全替代 Oracle 的 Data Grants
...
> 以下列出完整版本历史。PostgreSQL 版本自 v3.6.1 起发布；v3.6.1 之前的版本（v1.0.0 ~ v3.6.0）仅基于 Oracle。
...
| **Row-Level Security (RLS)** | 每张表创建 RLS 策略，通过 `app.current_agent_id` 会话变量实现行级隔离，替代 Oracle 的 Data Grants |
| **pgcrypto 列级加密** | LDAP BIND_CREDENTIAL、AGENT_CREDENTIALS 等敏感字段使用 pgcrypto AES 加密，替代 Oracle 的 DB_CRYPTO |
...
>
> PostgreSQL 通过 **共享角色 + RLS 策略** 模式在数据库层完全执行：
> - 所有 Business Agent 使用相同的共享数据库角色连接
> - Portal 用户连接后执行 `SET app.current_agent_id = '<agent_id>'` -> RLS 策略自动过滤可见行
> - Admin Dashboard 使用 AIADMIN 连接池（schema owner，不受 RLS 限制）
> - `connection.py` 自动路由：`set_agent_context()` -> 设置 `app.current_agent_id`；无上下文 -> AIADMIN 池
> - RLS 策略覆盖所有业务表（含 Portal 写操作授权及协作组访问）
>
> 验证结果（AGENT_001 vs AIADMIN）：
>
> | 表 | AIADMIN（全量） | AGENT_001（RLS） | 过滤率 |
> |----|----------------|---------------------|--------|
