# AI Agent Infra with PostgreSQL — 社区版 v3.7.0

**版本**: v3.7.0 | **日期**: 2026-06-18 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.7.0 是 AI Agent Infra 的 PostgreSQL 社区版 Bug 修复版本，包含 15 个 Bug 修复，涵盖 Portal 聊天、用户认证、图 API、PG 兼容性等关键问题。适配 PostgreSQL 18.3 生态：psycopg2 驱动、pgvector 向量搜索、Apache AGE 属性图、pg_cron 定时任务、PL/Python3u 存储过程、pgcrypto 数据库加密、Row Security Policies 行级安全。
...
| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.7.0** | 2026-06-18 | Bug 修复版本：15 个 Bug 修复，包括 Portal 聊天、用户认证、图 API、PG 兼容性 |
| **v3.6.1** | 2026-06-16 | PostgreSQL 社区版初始发布，功能对标 Oracle 社区版 v3.6.1 |
| Oracle v3.6.1 | 2026-06-18 | Bug fix：Portal 登录修复、图谱交互改进、文档一致性修正 |
...
| **引用分区** | 6 个子表继承 ENTITIES 分区策略，确保父子行物理同位 |
...
├────────────────────────────────────────────────────┤
│              数据库层（Database Layer）            │
│  30 表 · 22 PL/pgSQL 基础函数 + 78 API 函数        │
│  13 模式 · 13 pg_cron 作业 · 99+ 分区子表          │
│  分区 · 视图 · AGE 属性图 · pgvector · 全文索引    │
└────────────────────────────────────────────────────┘
```
...
## 循环工程（Loop Engineering）[NEW v3.7.0]
